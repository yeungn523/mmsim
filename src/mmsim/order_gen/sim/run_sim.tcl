# ---------------------------------------------------------------------------
# run_sim.tcl
# Run from: order_gen/sim/   (TCL lives here, sources are in ../)
...
vlog -reportprogress 300 -vlog01compat -work work ../tb_order_arbiter.v
...
#    catch lets this block fail silently in batch/headless mode
    ...
    add wave -unsigned /tb_order_arbiter/dut/grant_ptr
...
if {[file exists arbiter_log.csv]} {
    ...
    catch {exec python3 ../golden_order_arbiter.py arbiter_log.csv} result
    puts $result
} else {
