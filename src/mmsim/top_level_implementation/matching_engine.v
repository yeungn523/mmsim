...
module matching_engine #(
    ...
    // Head packet comes directly from the external FIFO's combinational output (showahead ON).
    wire [31:0] fifo_head_packet = fifo_dout;
    ...
    // Stage B pops the external FIFO when idle, the prior packet has retired, and data exists.
    assign fifo_rd_en = (b_state == kBIdle) && !fifo_empty && !b_to_c_valid;
    wire   fifo_pop   = fifo_rd_en;  // alias so all downstream references are unchanged
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // Set wins over clear: if we're issuing this cycle the flag stays high regardless
            // of command_ready, closing the one-cycle window where both fire simultaneously.
            if (bid_grant_c || bid_grant_b)
                bid_in_flight <= 1'b1;
            else if (bid_in_flight && bid_command_ready)
                bid_in_flight <= 1'b0;

            if (ask_grant_c || ask_grant_b)
                ask_in_flight <= 1'b1;
            else if (ask_in_flight && ask_command_ready)
                ask_in_flight <= 1'b0;
            ...
            case (b_state)
                kBIdle: begin
                    ...
                    if (!fifo_empty && !b_to_c_valid) begin
