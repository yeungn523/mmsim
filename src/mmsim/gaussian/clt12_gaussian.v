// clt12_gaussian.v
// CLT-12 Gaussian approximation using 12 parallel LFSR modules.
...
module clt12_gaussian (
    ...
    // -----------------------------------------------------------------------
    // Instantiate 12 Independent LFSR Modules
    // Using optimal sparse Galois polynomials from the Koopman database.
    // Seeds are scrambled via bitwise operations to guarantee independence.
    // -----------------------------------------------------------------------
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            begin
                ...
                // 1. Sum the top 16 bits of all 12 independent LFSR outputs
                ...
                // 2. Center the distribution (subtract exactly half of 12 * 65535)
                centered = $signed({1'b0, comb_sum}) - 22'sd393216;
                
                // 3. Scale down to Q4.12 by dividing by 16 (arithmetic shift right 4)
                shifted = centered >>> 4; 
