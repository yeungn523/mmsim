...
Builds the 256-layer Marsaglia and Tsang (2000) ziggurat decomposition for N(0, 1) and emits the
quantized tables in three companion formats: a Quartus MIF for M10K ROM initialization, a
behavioral Verilog include for ModelSim simulation, and a Python module that the gaussian_verify
golden model imports for cross-checking. All quantized representations use Q4.12 signed
fixed-point (16-bit) so the float, MIF, Verilog, and Python forms agree bit-for-bit with the
hardware ROM.
"""
...
def write_python_module(
    ...
    lines: list[str] = [
        '"""Stores Q4.12 Ziggurat lookup tables for the N(0, 1) golden model.',
        "",
        "Quantized representations match the hardware ROM bit-for-bit so the golden model and the",
        "M10K-backed FPGA path operate on identical data. Auto-generated by gen_ziggurat_tables.py;",
        "do not edit by hand.",
        '"""',
        "",
