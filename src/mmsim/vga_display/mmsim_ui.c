...
void debug_print_status(void)
{
    ...
    printf("          BID=%-4u  ASK=%-4u  SPREAD=%d  VOL=%-8u%s\n",
           bid, ask, (ask > bid) ? (int)(ask - bid) : -1, volume, volume_flag);
...
{
    int i, index, visible_minimum, visible_maximum, range, new_minimum, new_maximum, minimum_difference, maximum_difference;
    int n = candle_count;
    visible_minimum = 999;
    visible_maximum = 0;
    if (n < 1)
...
void update_candle(void)
{
    ...
    if (++window_tick >= TICKS_PER_CANDLE)
    {
        ...
        // Per-candle volume
        {
            uint32_t candle_volume = (OB_VOLUME >= volume_at_open)
        ...
        // Trader composition
        {
            ...
            else
            {
                ...
                composition_history[composition_head][3] = 255
                    - composition_history[composition_head][0]
...
void render_composition(void)
{
    ...
    for (y = COMPOSITION_Y0; y <= COMPOSITION_Y1; y++)
    {
        ...
        for (x = COMPOSITION_X0; x < COMPOSITION_X0 + COMPOSITION_WIDTH; x++)
        {
            ...
            int   history_index = (composition_head - n + slot + COMPOSITION_MAXIMUM_COLUMNS) % COMPOSITION_MAXIMUM_COLUMNS;
            int   cumulative    = 0;
...
int main(int argc, char* argv[])
{
    ...
    // Composition labels
    {
        ...
        int t;
        for (t = 0; t < 4; t++)
            VGA_text(label_character_column, label_rows[t], (char*)names[t]);
    }
