...
void update_axis(void)
{
    ...
    if (min_diff != 0 && min_diff > -8 && min_diff < 8) axis_min += (min_diff > 0) ? 1 : -1;
    else axis_min += min_diff / 8;
    if (max_diff != 0 && max_diff > -8 && max_diff < 8) axis_max += (max_diff > 0) ? 1 : -1;
...
void dummy_tick(void)
{
    ...
    {
        int w = mp - (3 + rand() % 10);
        if (w >= 0 && w < NUM_LEVELS) ob_mem[w] += 60 + rand() % 100;
    }
    if (rand() % 35 == 0)
    {
        int w = mp + (3 + rand() % 10);
        if (w >= 0 && w < NUM_LEVELS) ob_mem[400 + w] += 60 + rand() % 100;
    }
...
void update_candle(void)
{
    ...
    if (++window_tick >= TICKS_PER_CANDLE)
    {
        ...
        // Trader composition.
        {
            ...
            comp_hist[comp_head][3] = 255 - comp_hist[comp_head][0] - comp_hist[comp_head][1] - comp_hist[comp_head][2];
        }
...
void render_topbar(void)
{
    ...
    int pct10     = (int) ((int) (exec - open_price) * 1000 / (int) (open_price ? open_price : 1));
        ...
        for (x = CANDLE_X0; x < CANDLE_X0 + CANDLE_W - 1; x++)
        {
            ...
            // Interpolates close_y between adjacent candle centres at this x for area shading.
            {
                ...
                if (slot_i >= 0 && slot_i < n)
                {
                    ...
                    int y_next;
                    if (slot_i + 1 < n) y_next = c_close_y[slot_i + 1];
                    else y_next = cur_close_y;

                    int x_next = x_cur + SLOT;
                    int dx     = x_next - x_cur;

                    int interp_y;
                    if (dx > 0) interp_y = y_cur + ((y_next - y_cur) * (x - x_cur)) / dx;
                    else interp_y = y_cur;
            ...
            else
            {
                ...
                if (n > 0 && slot_i >= 0 && slot_i < n && slot_x < BODY_W)
                {
                    if (y >= c_high[slot_i] && y <= c_low[slot_i])
                    {
                        ...
                        else if (slot_x == BODY_W / 2) col = c_wick_col[slot_i];  // Wick overrides shading.
                    }
                }
...
void render_comp(void)
{
    int col, x, y;
    int n                                              = comp_count;
    static const short trader_colors[NUM_TRADER_TYPES] = {col_noise, col_mm, col_momentum, col_value};
...
int main(void)
{
...

    {
        ...
        int label_char_col = ((COMP_X0 + COMP_W - COMP_LABEL_W) >> 3) + 1;  // Char col of the label strip.
        {
            ...
            for (t = 0; t < 4; t++) VGA_text(label_char_col, label_rows[t], (char*) trader_names_top_to_bot[t]);
        }
    }
